...
from dataset.test_dataset import TestDataset, LowLightTestDataset, get_color_and_struct
...
def lowlight_enhance(swir, lowlight, img_name, pipe, args, consistent_start=None):
    _, _, H, W = lowlight.shape
    ...
    swir_struct, swir_color = swir_struct.cuda(), swir_color.cuda()
    fidelity_input = torch.cat([lowlight, swir_struct, swir_color], dim=1)

    if args.tiled:
        lowlight_patches, num_h, num_w, new_h, new_w = crop_parallel(lowlight, args.tile_size, args.tile_stride)
        swir_struct_patches, num_h, num_w, new_h, new_w = crop_parallel(swir_struct, args.tile_size, args.tile_stride)
        swir_color_patches, num_h, num_w, new_h, new_w = crop_parallel(swir_color, args.tile_size, args.tile_stride)
        fidelity_input_patches, num_h, num_w, new_h, new_w = crop_parallel(fidelity_input, args.tile_size, args.tile_stride)
        lowlight_patches_list = torch.split(lowlight_patches, 1, dim=0)
        swir_struct_patches_list = torch.split(swir_struct_patches, 1, dim=0)
        swir_color_patches_list = torch.split(swir_color_patches, 1, dim=0)
        fidelity_input_patches_list = torch.split(fidelity_input_patches, 1, dim=0)
        out_list = []
        for ind, (lq2_, lq1_struct_, lq1_color_, fidelity_input_) in enumerate(zip(lowlight_patches_list, swir_struct_patches_list, swir_color_patches_list, fidelity_input_patches_list)):
            set_seed(args.seed)
            out = pipe.run(lq2=lq2_, lq1_mscn_norm=lq1_struct_, lq1_color=lq1_color_, tiled=args.tiled, tile_size=args.tile_size, tile_stride=args.tile_stride, cond_fn=cond_fn, fidelity_input=fidelity_input_, consistent_start=consistent_start)
            out_list.append(out)
    ...
    out = out[:, :, :H, :W]
    swir = swir[:, :, :H, :W]
    lowlight = lowlight[:, :, :H, :W]
    swir_struct = swir_struct[:, :, :H, :W]
    swir_color = swir_color[:, :, :H, :W]

    save_image((out + 1) / 2, '{}/{}_out.png'.format(args.output, img_name))
    if args.save_all:
        save_image(swir, '{}/{}_swir.png'.format(args.output, img_name))
        save_image(lowlight, '{}/{}_lowlight.png'.format(args.output, img_name))
        save_image(swir_struct, '{}/{}_swir_struct.png'.format(args.output, img_name))
        save_image(swir_color[:, :1, :, :], '{}/{}_swir_color_u.png'.format(args.output, img_name))
        save_image(swir_color[:, 1:, :, :], '{}/{}_swir_color_v.png'.format(args.output, img_name))
...
parser.add_argument("--ckpt", type=str, default='ckpts/ultrafusion.pt', help='path to controlnet checkpoint')
...
try:
    ...
    print("loaded fidelity encoder")
except FileNotFoundError:
    print("WARNING: ckpts/fcb.pt not found, fidelity encoder disabled (ok for non-tiled or without --tiled)")
    fidelity_encoder = None
### load diffusion
...
### load flow model (only needed for MEF prealign)
if args.dataset != 'LowLight':
    ...
    flow_model = None
    print("LowLight mode: skip RAFT loading")
cond_fn = None
...
if args.dataset == 'LowLight':
    dataset = LowLightTestDataset(args.data_dir)
else:
